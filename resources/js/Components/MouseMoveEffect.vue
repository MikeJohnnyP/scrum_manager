...
import { useMouseMoveStore } from '../stores/mouseMoveStores';

const cursorOutline = ref(null);
const cursorDot = ref(null);
const loading = ref(null);
const store = useMouseMoveStore();
...
onMounted(() => {
    ...
    gsap.set(cursorDot.value, { scale: 0.04 });
    ...
    xDTo = gsap.quickTo(cursorDot.value, 'left', { duration: 0.6 });
    yDTo = gsap.quickTo(cursorDot.value, 'top', { duration: 0.6 });
...
watch(() => store.cursorShape, (shape) => {
    gsap.to(cursorOutline.value, {
        ...
        duration: 0.3,
        scale: shape.width === defaultWidth && shape.height === defaultHeight ? 0.5 : 1,
    });
    gsap.to(cursorDot.value, {
        ...
        borderRadius: shape.borderRadius,
        duration: 0.3,
        scale: shape.width === defaultWidth && shape.height === defaultHeight ? 0.04 : 1,
    });
});
